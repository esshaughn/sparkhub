...
test('leaving for Google saves the draft; cancelling brings it back', async ({ browser }) => {
  ...
  try {
    ...
    // They cancel at Google: back to "Look good?" with the draft and photo, URL cleaned up
    await page.goto('/?error=access_denied&error_description=cancelled');
    await expect(page.getByRole('heading', { name: 'Look good?' })).toBeVisible();
    await expect(page.locator('[data-screen-label="New spark"]')).toContainText(title);
    const review = page.locator('[data-screen-label="New spark"]');
    await expect(review.locator('div[style*="blob:"]')).toHaveCount(1);                 // the photo came back too
    await expect(review).not.toContainText('None');
    await expect(page.getByText('Google sign-in didn’t finish')).toBeVisible();
...
test('coming back signed in posts the saved draft', async ({ browser }) => {
  ...
  try {
    await page.evaluate(({ k, title }) => sessionStorage.setItem(k, JSON.stringify({
      at: Date.now(), stage: 'link', why: 'post', anonId: null, name: 'Tester', mergeToken: null,
...
test('Google account that already has an account: sign in to it instead', async ({ browser }) => {
  ...
  try {
    await page.evaluate((k) => sessionStorage.setItem(k, JSON.stringify({
      at: Date.now(), stage: 'link', why: 'keep', anonId: 'someone', name: '', mergeToken: '00000000-0000-0000-0000-000000000000', draft: null
    })), RESUME_KEY);

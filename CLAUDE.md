...
- Groups: members see their groups' ideas; `open_idea()` records an idea link in `link_access`, which lets that visitor see that one idea. `groups.code` isn't a readable column (only `group_code()` for admins). Group photos change only through `set_group_photo()` (admins, and only to a photo in their own storage folder). Memberships are created only by `create_group()` / `join_group()`, and members can update only their own `last_seen_at`. Joining, starting a group and posting require a signed-in (non-anonymous) account (`is_signed_in()`).
